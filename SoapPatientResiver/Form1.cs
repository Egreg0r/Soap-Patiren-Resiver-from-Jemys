/* 
==============ПАТЧРЕПОРТ=====
1.0.0.149 Outcomming table, date fact and timefact теперь подтягиваются из receptionCondition
*/
...
/*            if (result <= 0)
            ...
            {*/
                if (m == 1)
                {
                    ...
                    send = yesterday.ToString("dd-MM-yy 23:00");
                    DateTime daybeforeyesterday = ctoday.AddDays(-3);
                    sbegin = daybeforeyesterday.ToString("dd-MM-yy 23:00");
                }
                else
                {
                    DateTime yesterday = ctoday;
...
//            }
            ...
            //0,4
            // Начало формирования CSV файлов
            for (long jo = 0; jo < 2; jo++)
            {
                ...
                csv_init[0] = "num_dest;date_dest;form_pom;mo_src;date_fact;time_fact;vpolis;spolis;npolis;smo;ds_priem;prof_k;depart;num_hist;type_gosp;subj_rf;fam;im;ot;pol;dr;usl_ok\n";
                ...
                csv_init[1] = "form_pom;date_fact;time_fact;vpolis;spolis;npolis;smo;ds_priem;prof_k;depart;num_hist;type_gosp;subj_rf;fam;im;ot;pol;dr;is_vakcin;stat_zab;orit\n";
                ...
                for (int i = 0; i < PatientsIds.Length; progressBar.Value = prbegin + (20 * (i++ + 1)) / PatientsIds.Length)
                {
                    ...
                    {
                        //mo_src
                        if (mo_src == "250200") mo_src = "250203";//замена кода роддома на код поликлиники
                        CSVfile += nz(mo_src) + ";";
                    }
                    //date_fact
                    //time_fact
                    ...
                    if (tmp != null)
                    {
                        DateTime s = new DateTime(1970, 1, 1, 12, 0, 0);
                        s = s.AddMilliseconds(double.Parse(tmp));
                    ...
                    string stacionar_vid = ""; //тип стационара
                    ...
                    else if (department_code == "3021")// day stac pat
                    {
                        CSVfile += "5;";
                        stacionar_vid = "2;";
                        department_code = "302";
                    ...
                    else
                    {
                        ...
                        if (tmp != null)
                        {
                            ...
                            CSVfile += ";";
                    if (jo == 0)
                        CSVfile += stacionar_vid;

                    //is vakcin
                    //stat_zab
                    //orit
                    // дописываем данные по COvid19 в файл extr помощи (Эти поля заполняются только для диагнозов «Пневмония». )
                    if (jo == 1)
                    {
                        CSVfile += ("0;0;0");
                    }
            ...
            for (long i = 0; i < PatientInDepartmentAppointments.Length; i++)
            {
                ...
                //date_fact
                //time_fact
                GetItemValueByQuery(ref tmp, "/mis/rest/queries?method=getQueryXmlResult&query=SELECT history.receptionCondition.receptionDate FROM PatientInDepartmentAppointment p WHERE p.id=" + PatientInDepartmentAppointmentsId, "//gregorian-calendar/time"); // берет дату попадения на ресепшен (Бред какойто)
                if (tmp != null)
                {
                    DateTime s = new DateTime(1970, 1, 1, 12, 0, 0);
                    s = s.AddMilliseconds(double.Parse(tmp));
                ...
                /*   По всей видимости подтягивает дату и время когда пациент лег на койку.(!!!!!!!!!!
                ...
                {
                    System.DateTime min = new DateTime();
                    min = System.DateTimeOffset.Parse(tmp).DateTime;
                    CSVfile += min.ToString("dd.MM.yyyy;HH:mm;");
                }
                else CSVfile += ";";
                */
                //date_out
        ...
        private void XmlPrik_Click(object sender, EventArgs e)  // Формирование запроса на прикрепление пациента
        {
            var date = DateTime.Today.ToString("yyyy-MM-dd");
            string fileName = "250200_" + date +".att"; // переменная имени основного файла
            XDocument docXml;  // документ содержет полученые данные о Пациенте

            WebRequest request; //Запрос к базе
            WebResponse response; // ответ из базы
            List<string> paId; // коллекция patientAttachment.id со статусом Attached (активна)

            request = WebRequest.Create("http://" + cmbMis.Text  + "/mis/rest/queries?method=getQueryXmlResult&query=Select%20pa.id%20FROM%20PatientAttachment%20pa%20WHERE%20status=%27ATTACHED%27");
            response = request.GetResponse();

            using (Stream stream = response.GetResponseStream())
            {
                var paIdXml = XDocument.Load(stream);
                var m = from x in paIdXml.Element("list").Elements("long") select x.Value;
                paId = m.ToList();
                stream.Close();
            }
            ...
            docXml = new XDocument( 
                        ...
                        new XElement("ZL_LIST",   //создание шапки документа
                            new XElement("ZGLV",
                                new XElement("DATA", date),
                                new XElement("FILENAME", "250200_" + date),
                                new XElement("CODO", "250200"),
                                new XElement("CODMSS", "1"),
                                new XElement("TATT", "3")
                                //new XElement("NZAP", paId.Count)
                                )
                             )     
                        );
            //foreach (var n in paId)
            //{
            progressBar.Visible = true;
            progressBar.Maximum = 60 + paId.Count;
            progressBar.Value = 0;
            progressBar.Increment(20);

            var n = 1; // номер записи в реестре
            for (int i = 0; i < paId.Count; i++, progressBar.Increment(1))
            {

            XDocument xmlreq = XDocument.Load("http://" + cmbMis.Text  + "/mis/rest/queries?method=getQueryXmlResult&query=Select%20pa,%20pa.personalData,%20pa.doctor,%20pa.personalData.identityCard,%20pa.personalData.payment,%20pa.personalData.sex%20%20FROM%20PatientAttachment%20pa%20WHERE%20pa.id=" + paId[i]);
                //var i = 0;

                try
                {
                    DateTime bDate = DateTime.SpecifyKind(new DateTime(1970, 1, 1).AddMilliseconds(Convert.ToDouble(xmlreq.Descendants("birthDate").Elements("time").FirstOrDefault().Value)), DateTimeKind.Utc).ToLocalTime(); //перевод милисекунд ДР в дату ДР
                    DateTime dBeg = DateTime.SpecifyKind(new DateTime(1970, 1, 1).AddMilliseconds(Convert.ToDouble(xmlreq.Descendants("attachDate").Elements("time").FirstOrDefault().Value)), DateTimeKind.Utc).ToLocalTime(); //перевод милисекунд даты регистрации в дату

                    if ((xmlreq.Descendants("payment").Elements("number").FirstOrDefault().Value.Length == 16) &&(xmlreq.Descendants("doctor").Elements("code").FirstOrDefault().Value.Length > 6 ))
                    {
                        //заполнение xml документа значениями из базы
                        docXml.Element("ZL_LIST").Element("ZGLV").AddAfterSelf(new XElement("ZAP",
                                            new XElement("NOMZ", n),
                                            new XElement("FAM", xmlreq.Descendants("lastName").FirstOrDefault().Value),
                                            new XElement("IM", xmlreq.Descendants("firstName").FirstOrDefault().Value),
                                            new XElement("OT", xmlreq.Descendants("firstName").FirstOrDefault().Value),
                                            new XElement("W", xmlreq.Descendants("sex").Elements("code").FirstOrDefault().Value),
                                            new XElement("DR", bDate.ToString("yyyy-MM-dd")),
                                            new XElement("ENP", xmlreq.Descendants("payment").Elements("number").FirstOrDefault().Value),
                                            //new XElement("SDOC", xmlreq.Descendants("identityCard").Elements("series").FirstOrDefault().Value),
                                            //new XElement("NDOC", xmlreq.Descendants("identityCard").Elements("number").FirstOrDefault().Value),
                                            new XElement("DBEG", dBeg.ToString("yyyy-MM-dd")),
                                            new XElement("SSAT", "2"),
                                            new XElement("KSPEC", xmlreq.Descendants("doctor").Elements("code").FirstOrDefault().Value),
                                            new XElement("NUCH", xmlreq.Descendants("kind").Elements("id").FirstOrDefault().Value),
                                            new XElement("PRATT", "2")
                                                ));
                ...
                {
                    //MessageBox.Show("patientattachmentId = "+paId[i], "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    Console.WriteLine("patientattachmentId = " + paId[i]);
                }
            }

            docXml.Element("ZL_LIST").Element("ZGLV").Element("TATT").AddAfterSelf(new XElement("NZAP", n-1)); // добавление элемента кол-ва записей в файле

            docXml.Save(fileName);
            progressBar.Increment(20);
            progressBar.Increment(20);
            progressBar.Visible = false;
            Console.WriteLine("Выгрузка закончена");
        }
